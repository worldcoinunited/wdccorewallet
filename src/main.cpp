...
{
	// WDC: totally different code, defined GetSubsidy in chainparams.
		// thanks to RealSolid for helping out with this code
	int64_t qSubsidy = 64*COIN;
	int blocks = nHeight - Params().nDiffChangeTarget;
	int weeks = (blocks / Params().patchBlockRewardDuration)+1;
	//for each week that has passed, decrease reward by 1%
	for(int i = 0; i < weeks; i++)  qSubsidy -= (qSubsidy/100);  

	return qSubsidy + nFees;	
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
	...
	// Worldcoin: Force DERSIG right away.
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if (chainActive.Tip()->GetBlockTime() > GetAdjustedTime() - Params().TargetSpacing(pindexBestHeader->nHeight) * 20 &&
...
{
    {
        ...
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * Params().TargetSpacing(pindexBestHeader->nHeight) * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
